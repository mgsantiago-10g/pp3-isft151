...
try
{
    ...
    if (gettype($user_id) != "string" || gettype($name) != "string" || gettype($surname) != "string" || gettype($dni) != "string" || gettype($email) != "string") {
        throw new Exception("Invalid data type", 1);
    }

    $userInformationHandler = new UserInformationHandler();
    $userInformationHandler->create($user_id, $name, $surname, $dni, $email);
